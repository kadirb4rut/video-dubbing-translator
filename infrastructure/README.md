...
After apply, use the Terraform outputs for `api_url`, `S3_BUCKET`, `SQS_QUEUE_URL`, `AWS_REGION`, and the generated network IDs. The API service is intentionally steady-state; the selected CPU or GPU worker service scales from zero on demand and returns to zero after the SQS queue, including in-flight work, has drained.
