...
3. Apply Phase A with `retain_legacy_rds=true`, `retain_legacy_api=true`, and
   `serverless_frontend_cutover=false`. The legacy CloudFront origin remains
   live until the new control plane has data and direct-route evidence.
4. Run the PostgreSQL dump/restore from a VPC-reachable task and validate row
   ...
   downloads.
5. Validate the API Gateway/Lambda route directly, then set
   `serverless_frontend_cutover=true` in a separate reviewed apply and verify
   the CloudFront routes before public traffic is moved.
6. Run the manual `AWS Serverless CPU Scale-to-Zero E2E` workflow. Evidence
   must show worker `0 → 1+ → 0`, a completed job, and a verified artifact
   download.
7. Perform a full idle audit: no legacy API task, no ALB, Aurora paused at
...
8. Only after rollback is no longer needed, run a separately reviewed cleanup
