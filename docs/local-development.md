...
The API image intentionally does not install ML dependencies. Build `backend/Dockerfile.worker` with `WORKER_REQUIREMENTS=requirements.worker-cpu.txt` for a lightweight transcription/stems/noise worker, or the GPU requirements file for Chatterbox. The CPU file selects CPU-only PyTorch wheels; the GPU file pins security-reviewed runtime versions and the Dockerfile installs `chatterbox-tts==0.1.7` without its upstream optional/UI dependency pins. The backend imports only `chatterbox.mtl_tts`, so the Gradio UI is not included. Keep the provider import smoke test and `pip-audit --local` green before enabling a GPU image. The Compose worker uses the ML requirements image with `CHATTERBOX_DEVICE=cpu` so local development can exercise TTS and dubbing without an AWS GPU; use a GPU-backed image and `CHATTERBOX_DEVICE=cuda` for production inference. Missing packages fail a job explicitly and release its reservation.

When translation credentials are unavailable, set `TRANSLATION_PROVIDER=fixture` for deterministic development smoke tests. The fixture preserves segment timing and fails on unknown text; it never silently treats source text as a successful translation. The production default is `TRANSLATION_PROVIDER=hymt2` with `TRANSLATION_MODEL=tencent/Hy-MT2-1.8B`; the full worker requirements image downloads the model lazily into the configured cache. `TRANSLATION_PROVIDER=configured-api` and `TRANSLATION_PROVIDER=aws-translate` remain explicit alternatives for comparison or fallback.
...
Browser acceptance coverage lives in `frontend/e2e/lingowave.spec.js` and uses Playwright against a running frontend and API. Run `cd frontend && npm run test:e2e`; set `E2E_FRONTEND_URL` when the frontend is not at `http://127.0.0.1:5173`. The default suite verifies account creation, protected-dashboard access, reload persistence, and billing-surface behavior. Media-provider workflows generate a short FFmpeg fixture, upload through the UI, wait for a real worker result, and assert downloadable transcript artifacts; enable them with `E2E_GPU_ENABLED=true` only when an eligible GPU worker and model cache are available. They are intentionally not part of the fast unit-test job because they require a browser installation and a running media stack. The manual `.github/workflows/browser-e2e.yml` workflow runs the suite against an externally started/deployed stack and uploads Playwright traces and reports.
