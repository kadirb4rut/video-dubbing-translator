...
import { apiFetch, apiUrl, uploadMedia, uploadVoice } from './api';
...
function AudioOverview({ file }) {
  const bars = [34, 48, 27, 62, 39, 74, 53, 31, 68, 46, 82, 58, 36, 66, 47, 72, 54, 30, 60, 42, 76, 51, 68, 35, 57, 44, 70, 52, 31, 64, 48, 78, 58, 37, 69, 50, 74, 41, 62, 46, 80, 55, 35, 66, 47, 72];
  return <section className="audio-overview" aria-label="Audio overview"><div className="audio-overview-head"><div><span className="section-kicker">Audio overview</span><strong>{file?.name || 'Source media'}</strong></div><span>{file ? `${Math.round(file.duration_seconds)}s` : 'Ready when you are'}</span></div>{file ? <><div className="timeline-ruler"><span>00:00</span><span>00:04</span><span>00:08</span><span>00:12</span><span>{`${Math.round(file.duration_seconds)}s`}</span></div><div className="waveform-track"><div className="waveform-bars">{bars.map((height, index) => <i key={index} style={{ height: `${height}%` }} />)}</div><span className="waveform-marker" /></div><div className="audio-overview-foot"><span><i className="legend-dot source" />Source audio</span><small>Segments appear after transcription</small></div></> : <div className="audio-overview-empty">Upload a file to preview its timing and audio shape.</div>}</section>;
}

function RecentProjectsPreview({ jobs, onOpenProjects }) {
  const label = operation => operation === 'dubbing' ? 'Video translation' : operation === 'transcription' ? 'Transcription' : operation === 'stems' ? 'Stem splitter' : operation === 'noise' ? 'Noise remover' : 'Voice studio';
  if (!jobs.length) return <section className="recent-projects empty-recent"><div><span className="section-kicker">Workspace</span><h2>Your recent projects will appear here.</h2><p>Start with a media file and the result will stay available in Projects.</p></div><button type="button" className="text-link" onClick={onOpenProjects}>Open Projects <span>→</span></button></section>;
  return <section className="recent-projects"><div className="section-heading"><div><span className="section-kicker">Workspace</span><h2>Recent projects</h2></div><button type="button" onClick={onOpenProjects}>View all <span>→</span></button></div><div className="project-table-head"><span>Name</span><span>Tool</span><span>Stage</span><span>Updated</span></div>{jobs.slice(0, 3).map(job => <div className="project-row" key={job.id}><div className="project-thumb"><span>{job.operation === 'dubbing' ? '▶' : '◌'}</span></div><div className="project-name"><strong>{label(job.operation)}</strong><b>{job.id.slice(0, 8)}</b></div><span>{label(job.operation)}</span><em className={job.state === 'completed' ? 'green' : job.state === 'failed' ? 'red' : 'blue'}>{job.state.replaceAll('_', ' ')}</em><time>{new Date(job.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</time></div>)}</section>;
}

function TranslationInspector({ target, setTarget, sourceLanguage, setSourceLanguage, preserveVoice, setPreserveVoice, keepBackground, setKeepBackground, lipSync, setLipSync, quality, setQuality, translateSubtitles, setTranslateSubtitles, voices, voiceId, setVoiceId, cost, estimate, estimateError, hasFile, busy, job, onStart }) {
  const [tab, setTab] = useState('translation');
  const voiceRequired = !voiceId;
  const mediaRequired = !hasFile;
  ...
  const tabs = [['translation', 'Translation'], ['voice', 'Voice'], ['subtitles', 'Subtitles'], ['advanced', 'Advanced']];
  return <aside className="config-rail inspector-rail"><div className="inspector-heading"><div><span className="section-kicker">Configuration</span><h2>Make it yours</h2></div><span className="inspector-status"><i />{activeJob ? 'Processing' : 'Draft'}</span></div><div className="inspector-tabs" role="tablist" aria-label="Translation settings">{tabs.map(([value, label]) => <button type="button" role="tab" aria-selected={tab === value} className={tab === value ? 'active' : ''} key={value} onClick={() => setTab(value)}>{label}</button>)}</div>{tab === 'translation' && <div className="inspector-section"><div className="config-group"><label>Source language</label><select value={sourceLanguage || ''} onChange={e => setSourceLanguage(e.target.value)}>{sourceOptions.map(([label, value]) => <option key={label} value={value}>{label}</option>)}</select></div><div className="config-group"><label>Target language</label><select value={target} onChange={e => setTarget(e.target.value)}>{Object.keys(languages).map(lang => <option key={lang}>{lang}</option>)}</select></div><div className="inspector-note"><span>◎</span><p>We’ll detect the original language and keep translated timing close to the source.</p></div></div>}{tab === 'voice' && <div className="inspector-section"><div className="config-group"><label>Voice profile</label><select value={voiceId} onChange={e => setVoiceId(e.target.value)}><option value="">Choose a consented voice</option>{voices.map(voice => <option key={voice.id} value={voice.id}>{voice.name}</option>)}</select></div><div className="toggle-group"><Toggle label="Preserve voice characteristics" checked={preserveVoice} onChange={setPreserveVoice} /><Toggle label="Keep background audio" checked={keepBackground} onChange={setKeepBackground} /></div><div className="inspector-note warning"><span>!</span><p>{voiceRequired ? 'Choose a consented voice profile before processing.' : 'Your selected voice will be applied to the translated speech track.'}</p></div></div>}{tab === 'subtitles' && <div className="inspector-section"><Toggle label="Include translated subtitles" checked={translateSubtitles} onChange={setTranslateSubtitles} /><div className="inspector-note"><span>CC</span><p>Subtitle output will be stored with the completed job when enabled.</p></div></div>}{tab === 'advanced' && <div className="inspector-section"><div className="config-group"><label>Output quality</label><select value={quality} onChange={e => setQuality(e.target.value)}><option value="balanced">Balanced (recommended)</option><option value="studio">Studio</option><option value="draft">Draft</option></select></div><div className="toggle-group"><Toggle label="Lip sync" premium checked={lipSync} onChange={setLipSync} disabled /></div><div className="inspector-note"><span>✦</span><p>Advanced timing and worker selection remain automatic for this workspace.</p></div></div>}<div className="cost-box inspector-cost"><div><span>Estimated cost</span><strong>{cost}<small>credits</small></strong></div><span className="cost-basis">{estimate ? `${estimate.duration_seconds.toFixed(1)}s · server-side estimate` : 'Upload media to calculate'}</span><p className={estimateError ? 'cost-error' : undefined}>{estimateError || 'Your balance is checked before a worker starts.'}</p></div><button className="button primary start-button" disabled={busy || mediaRequired || voiceRequired || estimatePending || unavailable || Boolean(activeJob)} onClick={onStart}>{busy ? 'Working…' : activeJob ? `${job.state.replaceAll('_', ' ')} · ${stageLabel(job.state)}` : unavailable ? 'Unavailable' : estimatePending ? 'Calculating…' : 'Start translation'} {!busy && !unavailable && !estimatePending && <span>→</span>}</button></aside>;
}

function TranslationWorkbench({ file, busy, onFile, onRemove, target, setTarget, sourceLanguage, setSourceLanguage, preserveVoice, setPreserveVoice, keepBackground, setKeepBackground, lipSync, setLipSync, quality, setQuality, translateSubtitles, setTranslateSubtitles, voices, voiceId, setVoiceId, cost, estimate, estimateError, job, recentJobs, onOpenProjects, onStart, onDownload, onPreview, onCancel }) {
  return <div className="translation-workbench"><div className="workbench-topline"><div className="breadcrumbs"><span>Video Translator</span><b>›</b><strong>New translation</strong></div><div className="workbench-top-actions"><span className="autosave"><i />Workspace synced</span><button type="button" className="secondary-action" onClick={onOpenProjects}>View projects</button></div></div><div className="workflow-steps"><span className="current"><b>1</b><span>Upload<small>Add your media</small></span></span><i /><span className={file ? 'current' : ''}><b>2</b><span>Configure<small>Set languages and voice</small></span></span><i /><span className={job?.state === 'completed' ? 'current' : ''}><b>3</b><span>Export<small>Review and download</small></span></span></div><div className="translation-layout"><div className="translation-editor"><section className="source-panel"><div className="source-panel-heading"><div><span className="section-kicker">Source media</span><h2>{file ? 'Ready for translation' : 'Start with a video'}</h2></div><span className={`media-state ${file ? 'ready' : ''}`}><i />{file ? 'Media ready' : 'Awaiting upload'}</span></div><UploadCard file={file} busy={busy} onFile={onFile} onRemove={onRemove} title="Drop your video here" /></section><AudioOverview file={file} />{job && <DownloadableResult job={job} sourceFile={file} onDownload={onDownload} onPreview={onPreview} onCancel={onCancel} />}</div><TranslationInspector target={target} setTarget={setTarget} sourceLanguage={sourceLanguage} setSourceLanguage={setSourceLanguage} preserveVoice={preserveVoice} setPreserveVoice={setPreserveVoice} keepBackground={keepBackground} setKeepBackground={setKeepBackground} lipSync={lipSync} setLipSync={setLipSync} quality={quality} setQuality={setQuality} translateSubtitles={translateSubtitles} setTranslateSubtitles={setTranslateSubtitles} voices={voices} voiceId={voiceId} setVoiceId={setVoiceId} cost={cost} estimate={estimate} estimateError={estimateError} hasFile={Boolean(file)} busy={busy} job={job} onStart={onStart} /></div><RecentProjectsPreview jobs={recentJobs} onOpenProjects={onOpenProjects} /></div>;
}
...
export function RealAppShell({ onExit, initialUser }) {
  ...
  const createVoice = async (event) => { event.preventDefault(); if (!voiceReference) return showToast('Choose a 3–90 second reference sample.'); setBusy(true); try { const created = await uploadVoice(voiceReference, { name: voiceName, declaration: voiceDeclaration, authorized: voiceAuthorized }); setVoices(current => [created, ...current]); setVoiceId(created.id); setVoiceName(''); setVoiceDeclaration(''); setVoiceReference(null); showToast('Consent recorded and voice profile secured.'); } catch (error) { showToast(error.message); } finally { setBusy(false); } };
    ...
    <div className="app-shell">
      ...
      <div className="app-content">
        <header className="app-header">
          <div className="header-context"><span className="header-context-dot" />Personal workspace</div>
          <div className="header-search" aria-label="Search projects, files, or languages"><span>⌕</span><span>Search projects, files, or languages</span><kbd>⌘ K</kbd></div>
        ...
        <main className="workspace">
          ...
          ) : activeTool === "Video Translator" ? (
            <TranslationWorkbench file={file} busy={busy} onFile={handleFile} onRemove={() => { setFile(null); setJob(null); setEstimate(null); setPreviewText(''); setPreviewArtifactId(null); }} target={target} setTarget={setTarget} sourceLanguage={sourceLanguage} setSourceLanguage={setSourceLanguage} preserveVoice={preserveVoice} setPreserveVoice={setPreserveVoice} keepBackground={keepBackground} setKeepBackground={setKeepBackground} lipSync={lipSync} setLipSync={setLipSync} quality={quality} setQuality={setQuality} translateSubtitles={translateSubtitles} setTranslateSubtitles={setTranslateSubtitles} voices={voices} voiceId={voiceId} setVoiceId={setVoiceId} cost={cost} estimate={estimate} estimateError={estimateError} job={job} recentJobs={recentJobs} onOpenProjects={() => setActiveTool('Projects')} onStart={start} onDownload={downloadArtifact} onPreview={previewArtifact} onCancel={cancelJob} />
          ) : (
            <>
