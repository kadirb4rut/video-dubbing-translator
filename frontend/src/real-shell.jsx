...
export function RealAppShell({ onExit, initialUser }) {
  ...
  const selectTool = (tool) => { setActiveTool(tool); setFile(null); setEstimate(null); setJob(null); setPreviewText(''); setPreviewArtifactId(null); };
  return (
          ...
          {activeTool === "Projects" ? (
            ...
            <>
              ...
              <div className="workspace-grid">
                <div>
                  <UploadCard file={file} busy={busy} onFile={handleFile} onRemove={() => { setFile(null); setJob(null); setEstimate(null); setPreviewText(''); setPreviewArtifactId(null); }} title={activeTool === "Transcription" ? "Drop audio or video here" : activeTool === "Stem Splitter" ? "Drop music or video here" : "Drop your video here"} />
...
function ConfigRail({ activeTool, operation, target, setTarget, sourceLanguage, setSourceLanguage, preserveVoice, setPreserveVoice, keepBackground, setKeepBackground, lipSync, setLipSync, quality, setQuality, stemCount, setStemCount, translateSubtitles, setTranslateSubtitles, voices, voiceId, setVoiceId, cost, estimate, busy, job, onStart }) {
  const voiceRequired = activeTool === 'Video Translator' && preserveVoice && !voiceId;
