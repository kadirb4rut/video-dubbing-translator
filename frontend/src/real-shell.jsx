...
export function RealAppShell({ onExit, initialUser }) {
  ...
  const start = async () => { if (!file && activeTool !== 'Voice Studio') return showToast('Choose media first.'); if (activeTool === 'Video Translator' && !voiceId) return showToast('Create or choose a consented voice profile before starting.'); if (activeTool === 'Voice Studio' && (!voiceId || !voiceText.trim())) return showToast('Choose a voice and enter text first.'); setBusy(true); try { await requestJob(activeTool === 'Voice Studio' ? { text: voiceText, voice_profile_id: voiceId } : {}); } catch (error) { const message = error.message.includes('Measured credit profile') ? 'Processing is temporarily unavailable until measured cost profiles are configured.' : error.message; setEstimateError(message); showToast(message); } finally { setBusy(false); } };
